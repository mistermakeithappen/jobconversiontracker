# Database Schema Fix - SQL Commands for Supabase

## How to Apply These Fixes

1. Go to your Supabase Dashboard
2. Navigate to the SQL Editor
3. Copy and paste each section below **one at a time**
4. Run each section separately to avoid timeout issues

---

## STEP 1: Add Missing Columns to pipeline_stages
...
## STEP 2: Add Missing Columns to integrations
...
## STEP 3: Create commission_assignments Table

```sql
-- Commission Assignments Table
...
## STEP 4: Create opportunity_commissions Table

```sql
-- Opportunity Commissions Table
...
## STEP 5: Create commission_eligibility_log Table

```sql
-- Commission Eligibility Log Table
...
## STEP 8: Verification (Run this last)

```sql
-- Verify everything was created successfully
SELECT 'pipeline_stages columns' as check_type, column_name, data_type
FROM information_schema.columns 
WHERE table_name = 'pipeline_stages' 
AND column_name IN (
  'is_completion_stage', 'ai_reasoning', 'is_revenue_recognition_stage'
)
...
1. Your pipeline manual settings should work without the `ai_reasoning` error
2. Commission calculation APIs should have the required tables
3. All missing columns and tables will be available
